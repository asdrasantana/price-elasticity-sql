...
select
	...
	case
		when abs(percent_order_count/nullif(percent_avg_price, 0)) > 1 then 'elastic'
		when abs(percent_order_count/nullif(percent_avg_price, 0)) = 1 then 'unit elastic'
		when abs(percent_order_count/nullif(percent_avg_price, 0)) < 1 then 'inelastic'
	end as sensitivity
from percent
